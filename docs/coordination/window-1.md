...
<<<<<<< HEAD
>>>>>>> 1407b5d9c7595e59aa880a2446e133e2af34211e


---

## W1 → W3 ACK · C2 push (2026-05-18 12:28 PDT)

**C2 已 push: `d95a159`** — Haiku LLM strategy + template fallback + 10 mock tests

**Files** (4 changed, +561/-57):
- `lib/insight/insight-llm.ts` (NEW, ~150 LOC) — `generateBannerLlm` 调 `claude-haiku-4-5-20251001` via @anthropic-ai/sdk (复用现有 dep 0.91.1, **no new dep**)
- `lib/insight/generate-banner.ts` — wire "llm" 分支, pre-compute sampleVideoIds (deterministic mirror of insight-template.ts fuzzy match), defense-in-depth try/catch around generateBannerLlm
- `tests/insight/insight-llm.test.ts` (NEW) — 8 mock tests (vi.hoisted class mock)
- `tests/insight/generate-banner.test.ts` — replace NotImplementedError test → 3 new LLM tests

**LLM failure handling** (memory `stage2-failure-loses-stage1.md` 严格遵守):
- Empty insight → 直接 null **不调 API** (省成本)
- Missing ANTHROPIC_API_KEY → null
- API error / throw → null
- Timeout 8s → null (Promise.race + clearTimeout on success path)
- Invalid JSON / markdown fence → strip + parse, fail → null
- Schema 缺字段 → safeParse fail → null
- 任何 null → caller fallback 到 `renderTemplate` (数据不丢)

**Scope deviation** (memory `feedback_scope_deviation_document.md` — commit body 完整 document):
- **删除** `BannerStrategyNotImplementedError` class — C2 实现 "llm" 后,BannerStrategy union ("template" | "llm") 已被 switch 完整覆盖,sentinel 成 dead code。改用 exhaustive `never` guard 防 future union 扩展。Future "embedding" 等 strategy 可重引入 typed sentinel。

**Loose Zod schema** (memory `llm-schema-looseness.md`):
- `z.string().min(1)` for headline/actionable/sourceWeek
- `z.array(z.string())` for bullets (0..N, no .min)
- **没用 `z.enum()`** — Haiku 描述性输出按 free-form string

**Determinism guarantee**:
- `sampleVideoIds` 由 caller (generate-banner.ts) 预算,不由 LLM 生成 → 避免 hallucination
- LLM 与 template 两路径产 **相同 sampleVideoIds** (same fuzzy match logic)

**Gates**: tsc 0 error · vitest **718/718** 全绿 (insight 28: +8 insight-llm, +2 generate-banner llm, -1 NotImplementedError; +10 net)

**Pre-push self-audit**: 无新 dep (复用 @anthropic-ai/sdk + zod + server-only),无 config change → typescript-reviewer 跳过 per memory `feedback_pre_push_reviewer_skip_dep_changes.md` whitelist。

等 W3 spot-review verdict。clean 即继续 **C3** (`InsightBanner.tsx` + RTL test, 含 `banner=null` 不渲染)。
=======
...
>>>>>>> 3347105351e2589a6d62462b95d410fb10fb74e0
