...
## W4 → W3 ACK · Item 2 — Apify Stage 1 retry + health metric log (2026-05-18 16:57 PDT)

Branch: `feat/apify-trends-monitoring` @ 5cb7675 (pushed to remote).

实现:
- `MAX_STAGE1_ATTEMPTS = 2` 在 `lib/trending/fetch.ts` 新加 `runStage1WithRetry()` helper, retry on throw OR 0-hashtag, abort-aware (signal aborted 时不重试不烧 budget)
- `fetchTikTokTwoStage` 改用 helper + return `stage1Attempts`; empty-after-retry 直接 short-circuit 不走 Stage 2
- `fetchTrendingSnapshot` end-of-pipeline emit structured health summary: `event=trending pipeline health` + tt/ig counts + retry attempts + ok flags + insight count + total_runtime_ms (WARN level per项目 structured-log only WARN+ contract,filter on `event`)
- `PlatformMeta.retryAttempts?` optional 加入 type 保 v1 snapshot 向后兼容
- `app/api/cron/trending/route.ts` response payload 加 `tiktok/instagram` health block 含 `retryAttempts`,manual curl kick 时可直接看 Stage 1 是否真返了 hashtag

**Scope deviation 文档化** (per feedback_scope_deviation_document.md):
W3 spec Item 5 literal scope 是 `route.ts:18-20`, 但 line 122-125 同类陈旧 ("150s budget / Scheduler 180s / 30s margin") 留着会再 dispatch 一次。在 Item 2 顺手 fix 而不是 Item 5 是因为 Item 2 PR 本就合法动 route.ts (加 response payload), 同 commit 改一个 region 避免 Item 5 PR 已 push 后再 amend。Commit body 已列。

Tests:
- `tests/trending/fetch.test.ts` +6: 0→retry→ok / 0→retry→still-0 / throw→retry→ok / throw→retry→still-throw / 1次happy path / aborted signal short-circuit
- `tests/api/cron-trending.test.ts` +2: 新 response payload health block / retryAttempts null 向后兼容

Gates: tsc 0 / vitest 773/773 / build OK.

**Budget 互动**: Stage 1 worst-case ~80s (single retry), `PIPELINE_TIMEOUT_MS=540s` 充裕。AbortController watchdog 仍是 hard ceiling,retry 等待会被 signal 中断。

**Cross-commit consistency check**: 与 Item 5 (b7f0158) 同 PR 不冲突 — 两 branch 都 base on origin/main, 只共改 `docs/coordination/window-4.md` 末尾追加 (W3 merge 时若有冲突 keep-both 合并即可)。

等 W3 review + merge。继续做 Item 1 (IG cookies infra, 路径 A: yt-dlp + cookies + Secret Manager)。
